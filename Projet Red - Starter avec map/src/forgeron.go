...
type PlanFabrication struct { Armure string; Materiaux map[string]int }
...
func afficherPlansFabrication() {
	...
	for i, plan := range plansFabrication {
		...
		premier := true
		for _, nom := range []string{"Fer", "Cuir", "Plume divine", "Essence divine"} {
			if n := plan.Materiaux[nom]; n > 0 {
				if !premier { fmt.Print(" • ") }
				fmt.Printf("%s%s x%d%s", blanc, nom, n, reset)
				premier = false
			}
		}
		fmt.Println()
...
func (c *Character) MenuForgeron() {
	...
	for {
		...
		case "0": return
		case "M": afficherPlansFabrication()
		case "1", "2", "3", "4", "5", "6": c.fabriquerArmure(plansFabrication[int(choix[0]-'1')])
...
func (c *Character) fabriquerArmure(plan PlanFabrication) {
	for nom, quantite := range plan.Materiaux {
		if c.Inventaire[nom] < quantite {
			messageErreur(fmt.Sprintf("Il manque %s : %d/%d.", nom, c.Inventaire[nom], quantite))
			return
		}
	}
	consommes := 0
	for _, quantite := range plan.Materiaux { consommes += quantite }
	if c.nombreObjets()-consommes+1 > c.LimitInventaire { messageErreur("Inventaire plein après fabrication."); return }
	for nom, quantite := range plan.Materiaux { c.removeInventory(nom, quantite) }
	c.addinventory(plan.Armure, 1)
	switch plan.Armure {
	case "Bottes de Gladiateur", "Bottes de Hermès", "Ailes d'Icare": messageSucces(plan.Armure + " ont été fabriquées !")
	default: messageSucces(plan.Armure + " a été fabriqué !")
	}
