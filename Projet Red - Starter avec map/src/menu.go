...
func (c *Character) MenuInitCharacter() {
	...
	for {
		...
		var nom string
		if _, err := fmt.Scan(&nom); err != nil { messageErreur("Saisie invalide."); return }
		valide := true
		for _, lettre := range nom { if !unicode.IsLetter(lettre) { valide = false; break } }
		if !valide { messageErreur("Utilisez uniquement des lettres."); continue }
	...
	for {
		...
		switch choix {
		case 1: c.Classe = "Sparte"
		case 2: c.Classe = "Athénien"
		default: messageErreur("Choisissez 1 ou 2."); continue
		}
		break
	}
	c.initCharacter()
...
func (c *Character) MenuPrincipal() {
	for {
		...
		switch choix {
		case 0: return
		case 1: c.displayInfo()
		case 2: c.accessInventory()
		case 3: c.MarketMenu()
...
func (c *Character) Menutest() {
	for {
		...
		case 1: c.MarketMenu()
		case 2: c.MenuTour()
		case 3: c.MenuMaisonsDieux()
		case 4: c.MenuPrincipal()
		default: messageErreur("Choix invalide.")
		}
	}
}
