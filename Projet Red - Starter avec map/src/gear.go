...
type Weapon struct { name string; damage, vitesse int }
...
type Armor struct { name string; Pv int; slot string; vitesse int }
...
func (c *Character) equipWeapon(nom string) {
	if _, existe := offgear[nom]; !existe || nom == "CDP" { messageErreur("Arme inconnue."); return }
	if c.Inventaire[nom] <= 0 { messageErreur("Vous ne possédez pas cette arme."); return }
	if c.ArmeEquipee == nom { messageErreur("Cette arme est déjà équipée."); return }
	c.ArmeEquipee = nom
	messageSucces(nom + " est équipé.")
}

func (c *Character) unequipWeapon() {
	if c.ArmeEquipee == "" { messageErreur("Aucune arme équipée."); return }
...
func (c Character) armorSpeedBonus() int {
	bonus := 0
	for _, nom := range c.ArmuresEquipee { bonus += defgear[nom].vitesse }
	return bonus
}

func (c Character) cdpStats() (int, int) {
	return c.Damage + offgear["CDP"].damage, c.Vitesse + offgear["CDP"].vitesse + c.armorSpeedBonus()
}

func (c Character) weaponStats() (int, int) {
	arme, existe := offgear[c.ArmeEquipee]
	if !existe || c.ArmeEquipee == "" { return 0, 0 }
	return c.Damage + arme.damage, c.Vitesse + arme.vitesse + c.armorSpeedBonus()
}

func (c *Character) weaponEquipmentMenu() {
	var armes []string
	for nom, quantite := range c.Inventaire {
		if _, ok := offgear[nom]; ok && nom != "CDP" && quantite > 0 { armes = append(armes, nom) }
	}
	sort.Strings(armes)
	titreEcran("ARMES")
	for i, nom := range armes {
		fmt.Printf("  %s[%d]%s %s%s%s  %s(+%d dégâts)%s\n", dore, i+1, reset, bleu, nom, reset, blanc, offgear[nom].damage, reset)
	}
	if len(armes) == 0 { fmt.Println("  Vous ne possédez aucune arme.") }
	...
	if strings.EqualFold(choix, "R") { c.unequipWeapon(); return }
	if choix == "0" { return }
	var numero int
	if _, err := fmt.Sscan(choix, &numero); err != nil || numero < 1 || numero > len(armes) { messageErreur("Choix invalide."); return }
	c.equipWeapon(armes[numero-1])
}

func (c *Character) equipArmor(nom string) {
	armure, existe := defgear[nom]
	if !existe { messageErreur("Armure inconnue."); return }
	if c.Inventaire[nom] <= 0 { messageErreur("Vous ne possédez pas cette armure."); return }
	if c.ArmuresEquipee == nil { c.ArmuresEquipee = make(map[string]string) }
	if c.ArmuresEquipee[armure.slot] == nom { messageErreur("Cette armure est déjà équipée."); return }
	...
	c.PvMax += armure.Pv
	if ancienne != "" { messageSucces(ancienne + " est remplacé par " + nom + ".") } else { messageSucces(nom + " est équipé.") }
}

func (c *Character) unequipArmor(slot string) {
	nom := c.ArmuresEquipee[slot]
	if nom == "" { messageErreur("Aucune armure équipée dans cet emplacement."); return }
	c.PvMax -= defgear[nom].Pv
	if c.Pv > c.PvMax { c.Pv = c.PvMax }
	delete(c.ArmuresEquipee, slot)
