...
func (c *Character) initCharacter() {
	...
	switch c.Classe {
	case "Athénien": c.PvMax, c.Damage = 50, 15
	case "Sparte": c.PvMax, c.Damage = 60, 10
	}
	c.Pv = c.PvMax / 2
	c.Money = 10
	c.Vitesse = 10
...
func (c Character) displayInfo() {
	...
	d, v := c.cdpStats()
	ligneInfo("CDP", fmt.Sprintf("%d dégâts • %d vitesse", d, v))
	if c.ArmeEquipee != "" {
		d, v = c.weaponStats()
		ligneInfo("Attaque d'arme", fmt.Sprintf("%d dégâts • %d vitesse", d, v))
	}
}
